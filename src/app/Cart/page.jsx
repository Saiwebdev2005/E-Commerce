...
function Page() {
  ...
  return (
    <div className='bg-c3 flex flex-col justify-center items-center'>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-center md:max-w-6xl min-h-screen py-2 px-4 md:px-0">
        {cart.length > 0 ? (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 w-full md:w-3/4">
              {cart.map((item, index) => (
                <div key={index} className="max-w-xs sm:max-w-sm md:max-w-sm rounded overflow-hidden shadow-lg m-4 bg-c3 transform transition duration-500 ease-in-out hover:scale-105 hover:shadow-2xl">
                  ...
                  <div className="px-4 sm:px-6 py-2 sm:py-4">
                    ...
                    <button onClick={() => removeFromCart(item)} className="mt-2 bg-red-500 text-white px-2 py-1 rounded">Remove</button> {/* Add a "Remove" button */}
                  </div>
                </div>
              ))}
            </div>
            <div className="fixed top-1/2 right-2 transform -translate-y-1/2 w-full md:w-1/4 flex flex-col items-center justify-center md:justify-start md:ml-8">
